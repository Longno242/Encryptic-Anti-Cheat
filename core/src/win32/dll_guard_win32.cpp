...
HMODULE WINAPI hooked_load_library_w(LPCWSTR name) {
    if (name != nullptr) {
        ...
        if (len > 0) {
            ...
            auto& enc = anticheat::Encryptic::instance();
            const auto& cfg = enc.config();
            const std::string base = anticheat::platform::to_lower(anticheat::platform::basename(utf8));
            bool blocked_module = false;

            for (const auto& blocked : cfg.blocked_modules) {
                if (base.find(anticheat::platform::to_lower(blocked)) != std::string::npos) {
                    blocked_module = true;
                    break;
                }
            }
            ...
            if (cfg.block_unknown_dlls) {
                bool allowed = cfg.allowed_modules.empty();
                if (!allowed) {
                    for (const auto& m : cfg.allowed_modules) {
                        if (base == anticheat::platform::to_lower(anticheat::platform::basename(m))) {
                            allowed = true;
                            break;
                        }
                    }
                ...
                if (!allowed) {
                    enc.report_violation({
                        anticheat::ViolationType::UnknownDll,
                        anticheat::ViolationSeverity::Critical,
                        "Blocked unauthorized LoadLibraryW",
                        utf8,
                    });
                    return nullptr;
                }
...
void DllGuard::scan() {
    for (const auto& mod : snapshot_modules()) {
        ...
        for (const auto& blocked : blocked_modules_) {
            if (key.find(platform::to_lower(blocked)) != std::string::npos) {
                Encryptic::instance().report_violation({
                    ViolationType::UnknownDll,
                    ViolationSeverity::Critical,
                    "Blocked cheat module loaded",
                    mod.path,
                });
                break;
            }
...
void DllGuard::start(bool, const std::vector<std::string>&, const std::vector<std::string>&) {}
void DllGuard::scan() {}
