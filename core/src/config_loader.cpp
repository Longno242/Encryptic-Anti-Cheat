...
bool load_config_from_json(const std::string& json, Config& out) {
    ...
    if (!guards.empty()) {
        ...
        out.threat_intel_guard = json_bool(guards, "threat_intel_guard", out.threat_intel_guard);
        out.injection_guard = json_bool(guards, "injection_guard", out.injection_guard);
        out.anti_bypass_guard = json_bool(guards, "anti_bypass_guard", out.anti_bypass_guard);
        out.hash_guard = json_bool(guards, "hash_guard", out.hash_guard);
        out.pipe_guard = json_bool(guards, "pipe_guard", out.pipe_guard);
        out.kernel_guard = json_bool(guards, "kernel_guard", out.kernel_guard);
    ...
    if (!intervals.empty()) {
        ...
        out.threat_intel_scan_interval_ms =
            json_uint(intervals, "threat_intel_scan", out.threat_intel_scan_interval_ms);
        out.injection_scan_interval_ms =
            json_uint(intervals, "injection_scan", out.injection_scan_interval_ms);
        out.anti_bypass_scan_interval_ms =
            json_uint(intervals, "anti_bypass_scan", out.anti_bypass_scan_interval_ms);
        out.hash_scan_interval_ms = json_uint(intervals, "hash_scan", out.hash_scan_interval_ms);
        out.pipe_scan_interval_ms = json_uint(intervals, "pipe_scan", out.pipe_scan_interval_ms);
        out.kernel_poll_interval_ms = json_uint(intervals, "kernel_poll", out.kernel_poll_interval_ms);
    ...
    const auto blocked_modules = json_string_array(json, "blocked_modules");
    if (!blocked_modules.empty()) {
        out.blocked_modules = blocked_modules;
    }
    const auto blocked_classes = json_string_array(json, "blocked_window_classes");
    if (!blocked_classes.empty()) {
        out.blocked_window_classes = blocked_classes;
    }
    const auto blocked_titles = json_string_array(json, "blocked_window_titles");
    if (!blocked_titles.empty()) {
        out.blocked_window_titles = blocked_titles;
    }
    const auto blocked_paths = json_string_array(json, "blocked_path_segments");
    if (!blocked_paths.empty()) {
        out.blocked_path_segments = blocked_paths;
    }
    const auto blocked_drivers = json_string_array(json, "blocked_drivers");
    if (!blocked_drivers.empty()) {
        out.blocked_drivers = blocked_drivers;
    }
    const auto blocked_pipes = json_string_array(json, "blocked_pipes");
    if (!blocked_pipes.empty()) {
        out.blocked_pipes = blocked_pipes;
    }
    const auto blocked_mutexes = json_string_array(json, "blocked_mutexes");
    if (!blocked_mutexes.empty()) {
        out.blocked_mutexes = blocked_mutexes;
    }
    const auto known_hashes = json_string_array(json, "known_cheat_hashes");
    if (!known_hashes.empty()) {
        out.known_cheat_hashes.clear();
        for (const auto& entry : known_hashes) {
            ...
            CheatHashEntry che;
            che.label = entry.substr(0, sep);
            che.sha256_hex = entry.substr(sep + 1);
            out.known_cheat_hashes.push_back(std::move(che));
        }
    }

    out.kick_on_critical = json_bool(json, "kick_on_critical", out.kick_on_critical);
    out.block_unknown_dlls = json_bool(json, "block_unknown_dlls", out.block_unknown_dlls);
    out.terminate_on_debugger = json_bool(json, "terminate_on_debugger", out.terminate_on_debugger);

    const auto kernel_block = extract_block(json, "kernel");
    if (!kernel_block.empty()) {
        out.kernel_protect_handles = json_bool(kernel_block, "protect_handles", out.kernel_protect_handles);
        out.kernel_block_drivers = json_bool(kernel_block, "block_drivers", out.kernel_block_drivers);
        out.kernel_block_modules = json_bool(kernel_block, "block_modules", out.kernel_block_modules);
        out.kernel_block_global_cheats =
            json_bool(kernel_block, "block_global_cheats", out.kernel_block_global_cheats);
        out.kernel_poll_interval_ms =
            json_uint(kernel_block, "poll_interval_ms", out.kernel_poll_interval_ms);
        if (const auto path = json_string(kernel_block, "device_path"); !path.empty()) {
            out.kernel_device_path = path;
        }
    }
