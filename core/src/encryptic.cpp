...
void Encryptic::start(const Config& config) {
    ...
    if (config_.blocked_processes.empty()) {
        config_.blocked_processes = {
            "cheatengine", "cheat engine", "x64dbg", "x32dbg", "ida", "ida64", "processhacker",
            "reclass", "httpdebugger", "ollydbg", "windbg", "immunitydebugger", "artmoney",
            "tsearch", "wemod", "xenos", "extremeinjector", "ghinjector", "injector",
            "trainer", "speedhack", "memoryeditor", "gameguardian", "dnspy", "de4dot",
            "scylla", "hxd", "hiew", "lordpe", "petools", "megadumper", "ksdumper",
        };
    }
    ...
    last_threat_intel_scan_ = now;
    last_injection_scan_ = now;
    last_anti_bypass_scan_ = now;
    last_hash_scan_ = now;
    last_pipe_scan_ = now;
    last_kernel_scan_ = now;
    ...
    if (config_.dll_guard) {
        ...
        dll_guard_->start(config_.block_unknown_dlls, config_.allowed_modules, config_.blocked_modules);
    ...
    if (config_.threat_intel_guard) {
        threat_intel_guard_ = std::make_unique<ThreatIntelGuard>();
        threat_intel_guard_->configure(
            config_.blocked_modules,
            config_.blocked_window_classes,
            config_.blocked_window_titles,
            config_.blocked_path_segments,
            config_.blocked_drivers);
        threat_intel_guard_->scan();
    }

    if (config_.injection_guard) {
        injection_guard_ = std::make_unique<InjectionGuard>();
        injection_guard_->configure(config_.blocked_processes);
        injection_guard_->scan();
    }

    if (config_.anti_bypass_guard) {
        anti_bypass_guard_ = std::make_unique<AntiBypassGuard>();
        anti_bypass_guard_->scan();
    }

    if (config_.hash_guard) {
        hash_guard_ = std::make_unique<HashGuard>();
        hash_guard_->configure(config_.known_cheat_hashes);
    }

    if (config_.pipe_guard) {
        pipe_guard_ = std::make_unique<PipeGuard>();
        pipe_guard_->configure(config_.blocked_pipes, config_.blocked_mutexes);
        pipe_guard_->scan();
    }

    if (config_.kernel_guard) {
        kernel_guard_ = std::make_unique<KernelGuard>();
        KernelGuardSettings ksettings;
        ksettings.enabled = true;
        ksettings.protect_handles = config_.kernel_protect_handles;
        ksettings.block_drivers = config_.kernel_block_drivers;
        ksettings.block_modules = config_.kernel_block_modules;
        ksettings.block_global_cheats = config_.kernel_block_global_cheats;
        ksettings.poll_interval_ms = config_.kernel_poll_interval_ms;
        ksettings.device_path = config_.kernel_device_path;
        kernel_guard_->configure(
            ksettings, config_.blocked_processes, config_.blocked_drivers, config_.blocked_modules);
        kernel_guard_->connect();
        kernel_guard_->scan();
    }
...
void Encryptic::stop() {
    ...
    threat_intel_guard_.reset();
    injection_guard_.reset();
    anti_bypass_guard_.reset();
    hash_guard_.reset();
    pipe_guard_.reset();
    if (kernel_guard_) {
        kernel_guard_->disconnect();
    }
    kernel_guard_.reset();
    vm_guard_.reset();
...
void Encryptic::run_scheduled_guards(std::chrono::steady_clock::time_point now) {
    ...
    if (threat_intel_guard_ && elapsed_ms(last_threat_intel_scan_, config_.threat_intel_scan_interval_ms)) {
        threat_intel_guard_->scan();
        last_threat_intel_scan_ = now;
    }

    if (injection_guard_ && elapsed_ms(last_injection_scan_, config_.injection_scan_interval_ms)) {
        injection_guard_->scan();
        last_injection_scan_ = now;
    }

    if (anti_bypass_guard_ && elapsed_ms(last_anti_bypass_scan_, config_.anti_bypass_scan_interval_ms)) {
        anti_bypass_guard_->scan();
        last_anti_bypass_scan_ = now;
    }

    if (hash_guard_ && elapsed_ms(last_hash_scan_, config_.hash_scan_interval_ms)) {
        hash_guard_->scan();
        last_hash_scan_ = now;
    }

    if (pipe_guard_ && elapsed_ms(last_pipe_scan_, config_.pipe_scan_interval_ms)) {
        pipe_guard_->scan();
        last_pipe_scan_ = now;
    }

    if (kernel_guard_ && elapsed_ms(last_kernel_scan_, config_.kernel_poll_interval_ms)) {
        kernel_guard_->scan();
        last_kernel_scan_ = now;
    }
...
#ifdef _WIN32
    if (config_.kick_on_critical && violation.severity == ViolationSeverity::Critical) {
